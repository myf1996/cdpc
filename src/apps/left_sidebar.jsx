...
const LeftSiderbar = (props) => {
    ...
    const [isOpenMoonsoonRainfallReport, setIsOpenMoonsoonRainfallReport] = useState(false);
    ...
    <>
      <div className="leftsidebar">
        <Button className="collapse-btn" onClick={toggleMonthlySummary}>Monthly Climate Summary</Button>
        <Collapse isOpen={isOpenMonthlySummary}>
        <Card>
          <CardBody>
            <ul>
              {monthlySummary.map((item) => (
                <li><a href={item.link}> {item.name} </a> {item.new ? <span style={{color:"red",fontWeight:500}}> NEW </span>: null} </li>
              ))
              }
            </ul>
          </CardBody>
      ...
      <Button className="collapse-btn" onClick={toggleClimateRecord}>Climate Record</Button>
      ...
      <Button className="collapse-btn" onClick={toggleEclipse}>Eclipse</Button>
      ...
      <Button className="collapse-btn" onClick={toggleLunar}>Lunar Calenders</Button>
      ...
      <Button className="collapse-btn" onClick={toggleSunRiseSet}>Sun Rise & Set</Button>
      ...
      <Button className="collapse-btn" onClick={toggleMoonsoonRainfallReport}>Moonsoon Rainfall Reports</Button>
...
// MonthlySummary 2021 update 	(not import)
// state climate		(not import)
// climate record		
// eclipse
// sun rise & set
// lunar
// climate summary
// MonthlySummary rainfall report
// new moon sighting	(not import)